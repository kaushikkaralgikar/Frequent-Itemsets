...
def hash_function(pair):
    sum_ =0
    for i in list(pair):
        sum_ += sum(hash(char) for char in i)
    return sum_ % BUCKET_SIZE

def get_candidate_itemsets(partition, support, data_size):
    ...
    for basket in baskets_list:
        ...
        for pair in combinations(basket, 2):
            bucket_number = hash_function(pair)
            bitmap[bucket_number] = (bitmap[bucket_number] + 1)
    
    singleton_list = []
    singleton_list = sorted(list(dict(filter(lambda elem: elem[1] >= ps_support, temp_counter.items())).keys()))
    ...
    while None is not candidate_list and len(candidate_list)>0:
        ...
        #print(sorted(list(filtered_dict)))
        #print(ps_support)
    ...
    for combination in candidate_itemsets:
        if set(combination).issubset(set(partition)):
            if combination not in temp_dict.keys():
                ...
                new_count = temp_dict.get(combination)+1
                temp_dict.update({combination:new_count})
    #print(temp_dict)
...
if __name__ == "__main__":
    ...
    input_file_path = "./Data/user_business.csv"
    output_file_path = "./Data/output_task2_full_file_try1.txt"
    filter_threshold = "70"
    support = "50"
    ...
    basket_rdd = data_rdd.map(lambda line : (line.split(",")[0], line.split(",")[1]))\
        .groupByKey().map(lambda items: (items[0], sorted(set(items[1]))))\
            .filter(lambda items: len(items[1])>int(filter_threshold))\
            .map(lambda itemsets: itemsets[1])

    data_size = basket_rdd.count()

    candidate_itemset = basket_rdd.mapPartitions(
        lambda partition: get_candidate_itemsets(partition, int(support), data_size)
